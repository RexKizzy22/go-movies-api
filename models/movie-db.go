...
// return one movie and err, if any
...
func (m *DBModel) Movies(genre ...int) ([]*Movie, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	if len(genre) > 0 {
		where = fmt.Sprintf(
			`WHERE id IN 
			(SELECT movie_id FROM movies_genres WHERE genre_id = %d)`, genre[0])
	}
	...
	for rows.Next() {
		...
		for genreRows.Next() {
			...
			if err != nil {
				log.Println("MovieGenre assignment")
				return nil, err
			}
			genres[mg.ID] = mg.Genre.GenreName
		}
...
func (m *DBModel) Genres() ([]*Genre, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m *DBModel) InsertMovie(movie Movie) error {
	...
	query := `
		INSERT INTO movies
			(title, description, year, release_date, 
				rating, runtime, mpaa_rating, created_at, updated_at) 
...
func (m *DBModel) UpdateMovie(movie Movie) error {
	...
	query := `
		...
		SET	title = $1, description = $2, year = #3, release_date = $4, 
			rating = $5, runtime = $6, mpaa_rating = $7, updated_at = $8 
		WHERE id = $9
	`
